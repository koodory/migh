...
function pageControl(){
	...
	/* 관리자의 경우 전체보기 버튼 보여주기 */
		...
		allBtn.css('display','block')
	      .removeClass('btn_space').addClass("bottom-mg-01");
...
function pageCreate(data){ // data: Json, number: Id판별 인덱스(j값)
   ...
   var title = $('<h1>').text(data.title).addClass('text-title')
				...
				$.each(result.data, function(index,obj){
					$.each(obj, function(index,data){
						if(userNo == data.memberNo && userLv == "NORMAL"){//글쓴이와 작성자가 일치할때만
							pageCreate(data, count); 
						}else if(userLv == "ADMIN" &&  menuFlag == 2 
							&& (!data.answer || data.answer == '답변 대기중입니다.')){//대기 상태				
							pageCreate(data, count);
						}else if(userLv == "ADMIN" && !(menuFlag == 2)){ //관리자일때만 전체 출력		
							pageCreate(data, pageSize);
						}
					});
				});
				currPageNo = pageNo;
