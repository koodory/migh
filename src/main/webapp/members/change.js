...
function btnPress(){
	if($('#id').val().length > 0 && $('#name').val().length > 0 && 
			...
			$("<div>").addClass("label-alarm red")
				.text("변경 가능한 정보는 전화번호, 암호, 생일입니다.").appendTo("#changeForm")
		tot_flag = true;
	}else{
			$("<div>").addClass("label-alarm blue")
				.text("회원 정보 변경 준비가 완료되었습니다.").appendTo("#changeForm")
		tot_flag = false;
	}
