...
public class AuthFilter implements Filter {
	...
	public void doFilter(
		...
		//    로그인 여부 검사한다.
		if (servletPath.startsWith("/qna/")){
			HttpSession session = httpRequest.getSession();
			MembersVo vo = (MembersVo)session.getAttribute("loginUser");
	    String member = vo.getId();

			if (session.getAttribute("loginUser") == null || member.equals("GUEST")) {
				String contextRoot= httpRequest.getContextPath(); // "/web01"
				httpResponse.sendRedirect(contextRoot + "/auth/login.ajax");
				return;
			}
		}
